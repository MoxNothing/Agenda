...
int main(int argc, char *argv[])
{
	...
	do
	{ /*Etiqueta para retornar al Menu recursivamente*/
		if (HayContactos(Contactos))
		{ /*Verifica si no hay contactos*/
			...
			switch (x)
			{ /*En este Men� se validan 2 opciones (Insertar, Salir)*/
			case 1:
		...
		else
		{
			...
			switch (x)
			{ /*Este Men� contiene m�s opciones (Insertar, buscar, listar)*/
			case 1:
				Insertar(Contactos);
				break;
			case 2:
...
int MenuSecundario()
{
	...
	cout << Regla << endl;
	cout << "|\tBienvenido a tu Agenda Electrnica (Contactos)\t    |" << endl;
	cout << Regla << endl;

	cout << "|\t\t\t\t\t\t\t    |" << endl;
	cout << "|\t\t      Si hay contactos"
			 << " (" << (ContactosRegistrados - ContactosEliminados) << ")\t\t    |" << endl;
	cout << "|\t\t\t\t\t\t\t    |" << endl;
...
void Insertar(struct Agenda Contactos[])
{
	...
	do
	{ /*Etiqueta de Men� para insertar contactos*/
		...
		{
			cout << "Lleg? al l?mite de contactos permitidos en la agenda." << endl
					 << endl;
		}
	} while (salir == 0);
...
void BuscarPorTelefono(struct Agenda Contactos[])
{
	...
	if (c == 0)
	{
		cout << "\n\tEl contacto \"" << PorTelefono << "\" no se ha encontrado." << endl
				 << endl;
	}
...
void BuscarPorCelular(struct Agenda Contactos[])
{
	...
	if (c == 0)
	{
		cout << "\n\tEl contacto \"" << PorCelular << "\" no se ha encontrado." << endl
				 << endl;
	}
...
void BuscarPorEmail(struct Agenda Contactos[])
{
	...
	if (c == 0)
	{
		cout << "\n\tEl contacto \"" << PorEmail << "\" no se ha encontrado." << endl
				 << endl;
	}
...
void Listar(struct Agenda Contactos[])
{
	...
	for (; i < ContactosRegistrados; i++)
	{
		...
		if (Contactos[i].Nombre != " ")
		{
			...
			cout << "\t\tFecha N.: " << Contactos[i].Fecha.Nacimiento << endl
					 << endl;
...
void Imprimir(struct Agenda Contactos[], int posicion)
{
	...
	cout << "\t\tFecha N.: " << Contactos[posicion].Fecha.Nacimiento << endl
			 << endl;
...
void Actualizar(struct Agenda Contactos[], int posicion)
{
	...
	{ // ActualizarOtroContacto:
		...
		case 1:
			...
			cout << "   Se ha actualizado de \"" << Nombre << "\" a "
					 << "\"" << Contactos[posicion].Nombre << "\"" << endl;
			break;
		case 2:
			cout << "\tDigite el nuevo tel?fono: ";
			cin >> Contactos[posicion].Telefono;
			cout << "   Se ha actualizado de \"" << Telefono << "\" a "
					 << "\"" << Contactos[posicion].Telefono << "\"" << endl;
			break;
		case 3:
			cout << "\tDigite el nuevo celular: ";
			cin >> Contactos[posicion].Celular;
			cout << "   Se ha actualizado de \"" << Celular << "\" a "
					 << "\"" << Contactos[posicion].Celular << "\"" << endl;
			break;
		case 4:
			cout << "\tDigite el nuevo email: ";
			cin >> Contactos[posicion].Email;
			cout << "   Se ha actualizado de \"" << Email << "\" a "
					 << "\"" << Contactos[posicion].Email << "\"" << endl;
			break;
		case 5:
			salir = Salir();
		default:
